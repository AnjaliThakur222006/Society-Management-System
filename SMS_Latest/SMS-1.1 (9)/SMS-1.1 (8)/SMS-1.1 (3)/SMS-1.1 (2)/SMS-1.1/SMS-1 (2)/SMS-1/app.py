...
def init_db():
    ...
    if user_count == 0:
        ...
        # Add sample complaints if none exist
        c.execute("SELECT COUNT(*) FROM complaints")
        complaint_count = c.fetchone()[0]
        if complaint_count == 0 and owner_resident_id:
            # Add sample complaints for owner
            c.execute("INSERT INTO complaints (resident_id, category, description, status, priority, ai_score) VALUES (%s, %s, %s, %s, %s, %s)",
                      (owner_resident_id, 'Water', 'There is water leaking from the pipe in bathroom', 'Open', 'Urgent', 35))
            c.execute("INSERT INTO complaints (resident_id, category, description, status, priority, ai_score) VALUES (%s, %s, %s, %s, %s, %s)",
                      (owner_resident_id, 'Noise', 'Loud music from neighbor during night hours', 'Resolved', 'Normal', 15))
            c.execute("INSERT INTO complaints (resident_id, category, description, status, priority, ai_score) VALUES (%s, %s, %s, %s, %s, %s)",
                      (owner_resident_id, 'Lift', 'Lift not working since yesterday', 'Open', 'Urgent', 30))

        if complaint_count == 0 and tenant_resident_id:
            # Add sample complaints for tenant
            c.execute("INSERT INTO complaints (resident_id, category, description, status, priority, ai_score) VALUES (%s, %s, %s, %s, %s, %s)",
                      (tenant_resident_id, 'Electricity', 'Power outage in the flat since morning', 'Open', 'Urgent', 32))
            c.execute("INSERT INTO complaints (resident_id, category, description, status, priority, ai_score) VALUES (%s, %s, %s, %s, %s, %s)",
                      (tenant_resident_id, 'Parking', 'Car blocking my parking slot', 'Assigned', 'Normal', 18))
            c.execute("INSERT INTO complaints (resident_id, category, description, status, priority, ai_score) VALUES (%s, %s, %s, %s, %s, %s)",
                      (tenant_resident_id, 'Other', 'Suggestion for improving common area lighting', 'Resolved', 'Normal', 5))
    ...
    # Add sample complaints if none exist (run regardless of user_count)
    c.execute("SELECT COUNT(*) FROM complaints")
    complaint_count = c.fetchone()[0]
...
def get_flats(block_id):
    ...
    c.execute("SELECT flat_number FROM flats WHERE block_id = %s AND status = 'Vacant'", (block_id,))
    flats = c.fetchall()

    conn.close()

    return jsonify([flat[0] for flat in flats])
...
def admin_add_flats():
    ...
    try:
        ...
        for flat_num in range(from_flat_num, to_flat_num + 1):
            ...
            c.execute("INSERT INTO flats (block_id, flat_number) VALUES (%s, %s)", (block_id, flat_number))
...
def admin_delete_bill():
    ...
    try:
        ...
        c.execute("SELECT id FROM maintenance_bills WHERE id = %s", (bill_id,))
        if not c.fetchone():
            conn.close()
            return jsonify({'success': False, 'message': 'Bill not found'})
        ...
        if c.rowcount == 0:
            conn.close()
            return jsonify({'success': False, 'message': 'Failed to delete bill'})
